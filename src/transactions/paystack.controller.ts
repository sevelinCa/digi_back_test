...
import {
  CreatePayStackSubAccountDTO,
  CreatePayStackTransactionCallbackUrlDTO,
  CreatePayStackTransactionDTO,
  InitializeSplitPaymentDTO,
...
export class PaystackController {
  ...
  async getAllAbandonedTransactionReferences() {
    try {
      const references =
        await this.paystackService.getAllAbandonedTransactionReferences();
      return {
        status: true,
  ...
  async createPaystackTransactionWithoutAuth(
    ...
    @Body() paystackDto: CreatePayStackTransactionCallbackUrlDTO
  ): Promise<any> {
    try {
      const result =
        await this.paystackService.createPaystackTransactionWithoutAuth(
          orderId,
          paystackDto
        );
      return {
  ...
  async createPaystackTransactionWithAuth(
    ...
    try {
      const result =
        await this.paystackService.createPaystackTransactionWithAuth(
          userId,
          orderId,
          paystackDto
        );
  ...
  async initializePayment(@Body() dto: InitializeSplitPaymentDTO) {
    try {
      const result = await this.paystackService.initializeSplitPayment(dto);
      return {
        status: true,
  ...
  async getSupportedBanks(@Query("country") country: string) {
    ...
    try {
      const banks =
        await this.paystackService.getPayStackSupportedBanksByCountry(country);
      return {
        status: true,
