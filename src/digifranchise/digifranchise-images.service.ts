...
import {
  ...
  type UpdateDigifranchiseGalleryImageDto,
} from "./dto/create-digifranchise-image.dto";
...
export class DigifranchiseImagesService {
  ...
  async createDigifrachiseServiceImage(
    ...
    createDigifranchiseGalleryImageDto: CreateDigifranchiseGalleryImageDto,
    ...
    const ownedDigifrachise = await this.ownedDigifranchisepRepository.findOne({
  ...
  async updateImage(
    imageId: string,
    updateDigifranchiseGalleryImageDto: UpdateDigifranchiseGalleryImageDto,
  ): Promise<DigifranchiseGalleryImage> {
    const existingImage = await this.digifranchiseImageshipRepository.findOne({
      where: { id: imageId },
    });
    if (!existingImage) {
  ...
  async getAllImages(): Promise<DigifranchiseGalleryImage[]> {
    return this.digifranchiseImageshipRepository.find({
