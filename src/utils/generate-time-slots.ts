function generateTimeSlots(
  ...
  skipUnavailableDays: boolean): string[] {
  const currentDate = new Date();
  currentDate.setHours(0, 0, 0, 0); // Reset time to midnight
  const daysOfWeek = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
  const currentDayIndex = daysOfWeek.indexOf(currentDay);
  const availableDays = schedule.availabilityWeekDays.map(day => day.day);
  const unavailabilityDates = schedule.unavailability.map(unavail => unavail.workingDate);
  ...
  for (let i = currentDayIndex; i < daysOfWeek.length + currentDayIndex; i++) {
    ...
    const availability = schedule.availabilityWeekDays.find(item => item.day === day);
    if (!availability) {
      continue;
    }

    availability.availabilityDayTime.forEach(slot => {
      const startTime = new Date(`${formatDate(currentDate)}T${slot.startTime}`);
      ...
      while (startTime < endTime) {
        ...
        startTime.setMinutes(startTime.getMinutes() + slotDuration + timeBetweenSlots);
      }
    });
  }
