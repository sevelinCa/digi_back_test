...
export class CustomerSubscriptionService {
  ...
  async createSubscription(
    ...
    const existingSubscription =
      await this.customerSubscriptionRepository.findOne({
        where: {
          userId: Equal(user.id),
          digifranchiseOwnerId: Equal(digifranchise.id),
        },
      });
    if (existingSubscription) {
      throw new HttpException(
        "User is already subscribed to this digifranchise",
        HttpStatus.CONFLICT,
      );
    }
  ...
  async getOneSubscription(id: string): Promise<CustomerSubscription | null> {
    return this.customerSubscriptionRepository.findOne({
  ...
  async getSubscribersByDigifranchiseId(
    ...
    return this.customerSubscriptionRepository.find({
      where: {
        digifranchiseOwnerId: Equal(digifranchiseId),
        deleteAt: IsNull(),
      },
    });
  }
