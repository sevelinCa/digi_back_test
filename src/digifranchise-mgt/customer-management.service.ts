...
export class CustomerManagementService {
  ...
  async createCustomer(
    ...
    createCustomerManagementDto: CreateCustomerManagementDto,
    ...
    const savedCustomer =
      await this.customerManagementRepository.save(newCustomer);
  ...
  async getOneCustomerById(
    ...
    return this.customerManagementRepository.findOne({
      where: { id: customerId, deleteAt: IsNull() },
    });
  }

  async updateCustomer(
    customerId: string,
    updateCustomerManagementDto: UpdateCustomerManagementDto,
  ): Promise<CustomerManagement> {
    ...
    if (!customer) {
      throw new NotFoundException(
        `customer with ID ${customerId} not found or has been soft deleted.`,
      );
    }

    this.customerManagementRepository.merge(
      customer,
      updateCustomerManagementDto,
    );
