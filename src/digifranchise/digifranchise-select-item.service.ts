...
export class DigifranchiseSelectItemService {
  ...
  async selectOrUnselectService(
    ...
    const ownedDigifranchise = await this.ownedDigifranchisepRepository.findOne(
      { where: { id: digifranchiseOwnedId } },
    );
    ...
    const existingSelection =
      await this.digifranchiseSelectItemRepository.findOne({
        where: {
  ...
  async selectOrUnselectProduct(
    digifranchiseOwnedId: string,
    digifranchiseProductId: string,
    userId: string,
  ): Promise<DigifranchiseSelectProductOrServiceTable> {
    console.log(
      `Attempting to select or unselect product with ownedId: ${digifranchiseOwnedId}, productId: ${digifranchiseProductId}, userId: ${userId}`,
    );

    // Check if the product exists
    const existingProduct = await this.digifranchiseProductRepository.findOne({
      where: { id: digifranchiseProductId },
    });
    ...
    console.log(
      `Existing selection found: ${existingSelection ? "Yes" : "No"}`,
    );
    ...
    const savedSelection =
      await this.digifranchiseSelectItemRepository.save(newSelection);
    console.log("Selection saved successfully");
    return savedSelection;
  }
  async getAllSelectedServices(
    userId: string,
  ): Promise<DigifranchiseSelectProductOrServiceTable[]> {
  ...
  async getAllUnSelectedServices(): Promise<
