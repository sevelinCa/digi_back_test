...
import {
  ...
  type AvailabilityDayTime,
  type AvailabilitySlotsTimeOneOne,
  type Unavailability,
...
export class AvailabilityController {
  ...
  async createNewAvailability(
    ...
    return await this.availabilityService.createNewAvailability(
      availabilityDto,
      ownedFranchiseId,
    );
  }
  ...
  async getAvailabilitySlotsByDateAndFranchise(
    ...
    return await this.availabilityService.getAvailabilitySlotsByDateAndFranchise(
      parsedDate,
      ownerFranchiseId,
    );
  }
  ...
  async bookSlotDetail(
    ...
    try {
      const updatedSlot = await this.availabilityService.bookSlotDetail(
        slotId,
        ownedFranchiseId,
      );
      return updatedSlot;
  ...
  async getAllAvailabilitySlotsByAndFranchise(
    ...
    try {
      const slots =
        await this.availabilityService.getAllAvailabilitySlotsByAndFranchise(
          ownerFranchiseId,
        );
      return slots;
    } catch (error) {
  ...
  async getAllSlotesInDateByDateAndFranchise(
    ...
    try {
      const parsedDate = new Date(date);
      return await this.availabilityService.getAllSlotesInDateByDateAndFranchise(
        parsedDate,
        ownerFranchiseId,
      );
    } catch (error) {
      throw new HttpException(
        "Error fetching availability time slots details",
  ...
  @ApiResponse({
    status: HttpStatus.OK,
    description: "All unavailabilities retrieved successfully.",
    type: [AvailabilitySlotsDetails],
  })
  ...
  async getUnavailbilityByFranchise(
    ...
    try {
      const unavailbilities =
        await this.availabilityService.getUnavailbilityByFranchise(
          ownerFranchiseId,
        );
  ...
  @ApiOperation({
    summary: "Delete all records related to an ownedDigifranchise by ID",
  })
  @ApiResponse({
    status: HttpStatus.OK,
    description:
      "All records related to the ownedDigifranchise deleted successfully.",
  })
  @ApiResponse({
    status: HttpStatus.NOT_FOUND,
    description: "OwnedDigifranchise not found or no records to delete.",
  })
  ...
  async deleteAllByOwnedDigifranchise(
    ...
    try {
      await this.availabilityService.deleteAllByOwnedDigifranchise(
