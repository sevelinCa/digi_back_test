...
import {
  ...
  UpdateDigifranchiseServiceOfferedDto,
} from "./dto/create-digifranchiseServiceOffered.dto";
...
import {
  ...
  UpdateDigifranchiseSubProductDto,
} from "./dto/create-digifranchise-SubProduct.dto";
...
export class DigifranchiseController {
  ...
  @ApiResponse({
    ...
    description: "All Digifranchise have been successfully retrieved.",
  })
  ...
  async ownDigifranchise(
    ...
    try {
      return await this.digifranchiseService.ownDigifranchise(
        userId,
        digifranchiseId,
      );
    } catch (error) {
  ...
  @Roles(RoleEnum.digifranchise_super_admin)
  @ApiOperation({
    summary: "GET ALL - Retrieve all Digifranchises owned by the user",
  })
  @ApiResponse({
    status: HttpStatus.OK,
    description:
      "All Digifranchises owned by the user have been successfully retrieved.",
  })
  @Get("get-owned-digifranchises")
  @HttpCode(HttpStatus.OK)
  async findAllOwnedDigifranchise(
    @Req() req: Request,
  ): Promise<DigifranchiseOwner[]> {
    const userId = (req.user as UserEntity).id;
  ...
  @ApiOperation({
    summary:
      "GET - Retrieve a single Digifranchise owned by the user by its ID",
  })
  @ApiResponse({
    status: HttpStatus.OK,
    description:
      "The Digifranchise owned by the user has been successfully retrieved.",
  })
  @Get("get-one-owned-digifranchise/:digifranchiseId")
  @HttpCode(HttpStatus.OK)
  async findOneOwnedDigifranchise(
    @Req() req: Request,
    @Param("digifranchiseId") digifranchiseId: string,
  ): Promise<DigifranchiseOwner | null> {
    const userId = (req.user as UserEntity).id;
    return this.digifranchiseService.findOneOwnedDigifranchiseByUserId(
      userId,
      digifranchiseId,
    );
  }
  ...
  @ApiResponse({
    status: HttpStatus.OK,
    description: "Digifranchise has been successfully retrieved.",
  })
  @ApiResponse({
    status: HttpStatus.NOT_FOUND,
    description: "Digifranchise not found.",
  })
  @Get("get-one-digifranchise/:digifranchiseId")
  async getDigifranchiseByDigifranchiseId(
    @Param("digifranchiseId") digifranchiseId: string,
  ): Promise<Digifranchise> {
    const digifranchiseOwner =
      await this.digifranchiseService.getDigifranchiseByDigifranchiseId(
        digifranchiseId,
      );
  ...
  @ApiResponse({
    ...
    description: "A new Digifranchise has been successfully created.",
  })
  ...
  async createDigifranchise(
    ...
    return this.digifranchiseService.createDigifranchise(
  ...
  @ApiOperation({
    summary:
      "GET - Retrieve selected products and services by owner and user ID",
  })
  @ApiResponse({
    status: HttpStatus.OK,
    description:
      "Selected products and services have been successfully retrieved.",
  })
  @ApiResponse({
    status: HttpStatus.NOT_FOUND,
    description: "No items found for the given owner and user.",
  })
  async getByOwnerAndUserId(
    ...
    const userId = (req.user as UserEntity).id;
    return this.digifranchiseService.getSelectedItemByOwnerAndUserId(
      ownerId,
      userId,
    );
  }
...
export class DigifranchiseOptionEndpoint {
  ...
  @ApiResponse({
    status: HttpStatus.OK,
    description: "Service category has been successfully retrieved.",
  })
  @ApiResponse({
    status: HttpStatus.NOT_FOUND,
  ...
  async getDigifranchiseServiceCategoryById(
    ...
    const category =
      await this.digifranchiseService.getDigifranchiseServiceCategoryById(
        serviceCategoryId,
      );
    if (!category) {
      throw new NotFoundException(
        `DigifranchiseServiceCategory with ID ${serviceCategoryId} not found`,
      );
    }
    return category;
  }
  ...
  @ApiResponse({
    status: HttpStatus.OK,
    description: "Service offered has been successfully retrieved.",
  })
  @ApiResponse({
    status: HttpStatus.NOT_FOUND,
    description: "Service offered not found.",
  })
  async getDigifranchiseServiceOfferedById(
    @Param("serviceOfferedId") serviceOfferedId: string,
  ): Promise<DigifranchiseServiceOffered> {
    const serviceOffered =
      await this.digifranchiseService.getDigifranchiseServiceOfferedById(
        serviceOfferedId,
      );
    if (!serviceOffered) {
      throw new NotFoundException(
  ...
  async getDigifranchiseProductById(
    ...
    const product =
      await this.digifranchiseService.getDigifranchiseProductById(productId);
    if (!product) {
      throw new NotFoundException(
        `DigifranchiseProduct with ID ${productId} not found`,
      );
    }
    return product;
  }
