...
export class ProfessionalBodiesService {
  ...
  async getProfessionalBodies(): Promise<ProfessionalBodyEntity[]> {
    ...
    const accreditationsByProfessionalBodyId = getAccreditationsFromDB.reduce((acc, accreditation) => {
    ...
    const professionalBodiesWithAccreditations = professionalBodies.map(professionalBody => ({
      ...professionalBody,
      accreditations: accreditationsByProfessionalBodyId[professionalBody.id] || []
    }));
