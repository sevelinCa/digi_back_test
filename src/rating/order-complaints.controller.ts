...
export class OrderComplaintsController {
    ...
    constructor(private readonly orderComplaintsService: OrderComplaintsService) { }

    @ApiOperation({ summary: 'CREATE - Create a new order complait' })
    @ApiResponse({ status: HttpStatus.CREATED, description: 'A new order complait has been successfully created.' })
