...
export class StaffManagementService {
  ...
  async getAllStaff(userId: string): Promise<StaffManagement[]> {
    return this.staffManagementRepository.find({
      where: { userId: Equal(userId), deleteAt: IsNull() },
    });
  }

  async getOneStaffById(staffId: string): Promise<StaffManagement | null> {
    return this.staffManagementRepository.findOne({
      where: { id: staffId, deleteAt: IsNull() },
    });
  }
  ...
  async updateStaff(
    ...
    const staff = await this.staffManagementRepository.findOne({
      where: { id: staffId, deleteAt: IsNull() },
    });
    if (!staff) {
      throw new NotFoundException(
        `staff with ID ${staffId} not found or has been soft deleted.`,
      );
    }
