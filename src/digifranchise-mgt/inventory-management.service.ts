...
export class InventoryManagementService {
  ...
  async createInventory(
    ...
    createInventoryManagementDto: CreateInventoryManagementDto,
    ...
    const savedInventory =
      await this.inventoryManagementRepository.save(newInventory);
    return savedInventory;
  }

  async getAllInventory(): Promise<InventoryManagement[]> {
    return this.inventoryManagementRepository.find({
  ...
  async getOneInventoryById(
    ...
    return this.inventoryManagementRepository.findOne({
      where: { id: inventoryId, deleteAt: IsNull() },
    });
  }
  ...
  async updateInventory(
    ...
    const inventory = await this.inventoryManagementRepository.findOne({
      where: { id: inventoryId, deleteAt: IsNull() },
    });
    if (!inventory) {
      throw new NotFoundException(
        `inventory with ID ${inventoryId} not found or has been soft deleted.`,
      );
    }

    this.inventoryManagementRepository.merge(
      inventory,
      updateInventoryManagementDto,
    );
