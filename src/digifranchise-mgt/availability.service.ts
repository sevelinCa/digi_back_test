...
export class AvailabilityService {
    ...
    try {
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (availabilityDto.availabilityWeekDays) {
          ...
          if (matchingDayOfWeek) {
            ...
            if (
              ...
              matchingDayOfWeek.availabilityDayTime.length > 0
            ) {
              ...
              const availabilityPromises: Promise<AvailabilityDayTime>[] =
                matchingDayOfWeek.availabilityDayTime.map((dayTimeDto) => {
                  return this.createAvailabilityDayTime(
                    owned,
                    savedWeekDay,
                    dayTimeDto,
                    availabilityDto,
        ...
        if (
          ...
          const unavailabilityPromises: Promise<Unavailability>[] =
  ...
  async bookSlotDetail(
    ...
    const updatedSlot =
      await this.availabilitySlotsDetailsRepository.save(slot);
  ...
  async updateAvailability(
    ...
    try {
      await this.availabilitySlotsDetailsRepository.delete({
        ownedDigifranchise: { id: ownedFranchiseId },
      });
      await this.availabilityDayTimeRepository.delete({
        ownedDigifranchise: { id: ownedFranchiseId },
      });
      await this.unavailabilityRepository.delete({
        ownedDigifranchise: { id: ownedFranchiseId },
      });
      await this.availabilityWeekDaysRepository.delete({
        ownedDigifranchise: { id: ownedFranchiseId },
      });
      await this.availabilityRepository.delete({
        ownedDigifranchise: { id: ownedFranchiseId },
      });
      const response = await this.createNewAvailability(
  ...
  async getWorkingHoursRange(ownerFranchiseId: string) {
    ...
    const allAvailabilitySlots =
      await this.availabilitySlotsDetailsRepository.find({
        where: {
